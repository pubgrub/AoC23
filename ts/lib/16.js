...
let lines = (0, fs_1.readFileSync)(fileName).toString().split('\n');
...
const nextX = { 0: 0, 1: 1, 2: 0, 3: -1 };
const nextY = { 0: -1, 1: 0, 2: 1, 3: 0 };
const beams = [{ x: 0, y: 0, d: 1 }];
const cache1 = [];
const seen = {};
// Part 1
let done = false;
...
// Part 2
const xmin = [0, width + 1, 0, -1];
const xmax = [width, width + 1, width, -1];
const ymin = [-1, 0, height + 1, 0];
const ymax = [-1, height, height + 1, height];
const sDir = [2, 3, 0, 1];
let routesToGoX = [];
let routesToGoY = [];
let routesToGoD = [];

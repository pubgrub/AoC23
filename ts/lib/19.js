...
function solve1(rules_, data_) {
    return data.reduce((acc, d) => acc + calcData(rules_, d), 0);
}
function calcData(rules_, d) {
    let func = rules_.in;
...
function solve2(rules_) {
    const minVals = [1, 1, 1, 1];
    const maxVals = [4000, 4000, 4000, 4000];
    return solve2Rec(rules_, 'in', minVals, maxVals);
}
function solve2Rec(rules_, str_, minVals_, maxVals_) {
    if (str_ === 'A')
        return minVals_.reduce((acc, m, i) => acc * (maxVals_[i] - m + 1), 1);
    if (str_ === 'R')
        return 0;
    const vI = { x: 0, m: 1, a: 2, s: 3 };
    let sum = 0;

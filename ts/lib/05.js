...
let seeds = [];
const maps = []; // maps[ conversion_pair][line][ dest, origin, groupLen]
let haveSeeds = false;
let idx = -1;
for (const l of inputLines) {
    ...
    if (!haveSeeds) {
        ...
        if (match === null) {
            console.error('Seed Match did not match');
            process.exit(1);
        }
        seeds = match === null || match === void 0 ? void 0 : match.map(s => Number(s));
        haveSeeds = true;
    }
    else if (l.includes('-')) {
        idx = maps.push([]) - 1;
        ...
        const match = l.match(numbersMatch);
        if (match === null) {
            console.error('Map Match did not match');
            process.exit(1);
        }
...
// Part 2
let rangesForSection = [];
let resultForSection = [];
const firstSeedRanges = [];

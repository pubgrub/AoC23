...
const button: device = { type: BUTTON, state: false, inputDev: [], inputSignals: [], outputDev: [] }
const output: device = { type: OUTPUT, state: false, inputDev: [], inputSignals: [], outputDev: [] }
const rx: device = { type: RX, state: false, inputDev: [], inputSignals: [], outputDev: [] }
const devices: Record<string, device> = { button, output, rx }

const types: Record<string, number> = { b: BROADCASTER, '%': FLIPFLOP, '&': CONJUNCTION }

const sourceDestConnections: Array<[string, string]> = []
inputLines.forEach((l) => {
  if (l.length > 0) {
    const dev: device = { type: -1, state: OFF, inputDev: [], inputSignals: [], outputDev: [] }
    let devName = ''
...
function solve (part: number): number {
  const signals: Array<[device, device, number]> = []
  let src: device, dest: device
  let sig: number
  ...
  for (let i = 0; i < maxI; i++) {
    ...
    if (rxCount === 1 && part === 2) return i
    if (i % 100000 === 0)console.log(i, rxCount)
  }
  return 0
}

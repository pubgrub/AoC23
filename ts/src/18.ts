...
function solve (dir_: string[], len_: number[]): number {
  const xOffset: Record<string, number> = { U: 0, R: 1, D: 0, L: -1 }
  const yOffset: Record<string, number> = { U: -1, R: 0, D: 1, L: 0 }

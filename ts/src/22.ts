...
if (test) console.log(brickListString(bricks))
  ...
  while (hasMoved) {
    ...
    for (let b1Idx = missing; b1Idx < bricks_.length; b1Idx++) {
      ...
      for (let b2Idx = 0; b2Idx < b1Idx; b2Idx++) {
        if (bricks_[b1Idx].zMin <= bricks_[b2Idx].zMin) continue
...
function xyCollision (b1: brick_, b2: brick_): boolean {
  if (b1.xMax < b2.xMin || b2.xMax < b1.xMin) return false
  if (b1.yMax < b2.yMin || b2.yMax < b1.yMin) return false
  return true
}

function brickListString (bricks_: brick_[]): string {
  let str = ''
  bricks_.forEach((b) => {
...
function solve1 (bricks_: brick_[]): number {
  let count = 0
...
function solve2 (bricks_: brick_[]): number {
  let count = 0
  for (let i = 0; i < bricks_.length; i++) {
    const bricks = JSON.parse(JSON.stringify(bricks_))
    bricks.splice(i, 1)

...
inputLines.forEach((l) => {
  ...
  if (m !== null) {
    const src: string = m[0]
    const dest: string[] = [...m.slice(1)]
...
const done: string[] = []
let maxLength = 0
const pathLength: Record<string, number> = {}
...
function dijkstra (grid: Record<string, string[]>, start: string, end: string): number {
  const myGrid = structuredClone(grid)
  const toCalc: string[] = [start]
...
function countMembers (grid_: Record<string, string[]>, start: string): number {
